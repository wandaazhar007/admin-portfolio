...
import type { BlogDetailResponse, BlogPayload } from "../../types/blog";
  ...
  const fetchData = async () => {
    try {
      ...
      const [blogResponse, categoriesResponse] = await Promise.all([
        api.get<BlogDetailResponse>(`/blogs/${uuid}`),
      ...
      setForm({
        title: blog.title,
        desc: blog.desc,
        categoryUuid: blog.categoryUuid,
        link: blog.link,
        author: blog.author,
      ...
      console.error(error);
      setSubmitError("Failed to load blog data.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className={styles.pageHeader}>
        <div>
          ...
          <p className={styles.pageDescription}>
            Update blog content, external link, category, and publish status.
          </p>
        </div>
